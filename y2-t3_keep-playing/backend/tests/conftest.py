...
# Force django-q2 into sync mode so enqueued email tasks run inline during
# tests. Must be set before django.conf.settings loads.
os.environ.setdefault('Q_SYNC', 'True')
...
def _clear_throttle_cache():
    """Reset DRF's throttle bucket between tests.

    Without this the new per-scope throttles (signup 5/hour, login_username
    10/hour) accumulate across tests and start returning 429 partway through
    a run.
    """
    cache.clear()
    yield
...
@pytest.fixture
def past_assigned_event(organiser_user, coach_user):
    today = date.today()
    now = timezone.now()
    return Event.objects.create(
        name='Past Swimming Lessons',
        sport='Swimming',
        role='Coach',
        date=today - timedelta(days=7),
        location='Aquatics Centre',
