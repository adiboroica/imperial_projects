...
@pytest.mark.django_db
class EventOffersTest:

    def test_get_event_offers(self, organiser_client, sample_event, coach_user):
        sample_event.offers.add(coach_user)
        response = organiser_client.get(f'/organiser/events/{sample_event.pk}/offers/')
        assert response.status_code == 200
        assert len(response.data) == 1
        assert response.data[0]['username'] == coach_user.username

    def test_get_event_offers_empty(self, organiser_client, sample_event):
        response = organiser_client.get(f'/organiser/events/{sample_event.pk}/offers/')
        assert response.status_code == 200
        assert len(response.data) == 0

    def test_get_event_offers_includes_rating(self, organiser_client, sample_event, coach_user):
        sample_event.offers.add(coach_user)
        response = organiser_client.get(f'/organiser/events/{sample_event.pk}/offers/')
        assert response.status_code == 200
        assert 'rating' in response.data[0]

    def test_get_event_offers_not_found(self, organiser_client):
        response = organiser_client.get('/organiser/events/99999/offers/')
        assert response.status_code == 404

    def test_other_organiser_cannot_see_offers(self, sample_event, coach_user):
        sample_event.offers.add(coach_user)
        other = User.objects.create_user(
            username='other_org_offers', password='pass', is_organiser=True,
        )
        Organiser.objects.create(user=other)
        token = Token.objects.create(user=other)
        client = APIClient()
        client.credentials(HTTP_AUTHORIZATION=f'Token {token.key}')
        response = client.get(f'/organiser/events/{sample_event.pk}/offers/')
