...
class CoachChecklistPage extends StatefulWidget {
  ...
  const CoachChecklistPage({
    super.key,
    required this.title,
    required this.exitGuardContent,
    required this.confirmTitle,
    required this.initialSelectedPks,
    required this.onSave,
  });

  @override
  State<CoachChecklistPage> createState() => _CoachChecklistPageState();
}

class _CoachChecklistPageState extends State<CoachChecklistPage> {
  List<User>? _coaches;
  ...
  void initState() {
    super.initState();
    _selectedPks = Set<int>.from(widget.initialSelectedPks);
    _loadCoaches();
  }
  ...
  Widget build(BuildContext context) {
    return ExitGuard(
      ...
      content: widget.exitGuardContent,
      child: Scaffold(
        appBar: AppBar(
  ...
  Future<void> _handleSave() async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (_) => ConfirmationDialog(title: widget.confirmTitle),
    );
