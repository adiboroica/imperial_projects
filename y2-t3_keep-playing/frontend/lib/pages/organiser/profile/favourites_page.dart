...
class _FavouritesPageState extends State<FavouritesPage> {
  ...
  void initState() {
    super.initState();
    _selectedPks = context.read<OrganiserCubit>().state.favourites.toSet();
    _loadCoaches();
  }
  ...
  Widget build(BuildContext context) {
    return ExitGuard(
      ...
      content: 'Any unsaved changes to your favourites will be lost.',
      child: Scaffold(
        appBar: AppBar(
  ...
  Future<void> _handleSave() async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (_) => const ConfirmationDialog(title: 'Save favourites?'),
    );
    ...
    try {
      ...
      final response = await apiOrganiser.updateFavourites(_selectedPks.toList());
