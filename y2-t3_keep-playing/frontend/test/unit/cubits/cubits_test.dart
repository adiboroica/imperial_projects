...
class MockOrganiserRepository extends Mock implements OrganiserRepository {}
...
void main() {
  late MockCoachRepository mockCoachRepository;
  late MockOrganiserRepository mockOrganiserRepository;

  setUp(() {
    mockCoachRepository = MockCoachRepository();
    mockOrganiserRepository = MockOrganiserRepository();
  });
  ...
  group('FeedCubit', () {
    test('initial state is DataInitial', () {
      when(() => mockCoachRepository.getFeedEvents())
          .thenAnswer((_) async => <Event>[]);
      final cubit = FeedCubit(coachRepository: mockCoachRepository);
    ...
    blocTest<FeedCubit, DataState<List<Event>>>(
      ...
      build: () {
        ...
        return FeedCubit(coachRepository: mockCoachRepository);
      },
  ...
  group('UpcomingJobsCubit', () {
    test('initial state is DataInitial', () {
      when(() => mockCoachRepository.getUpcomingJobs())
          .thenAnswer((_) async => <Event>[]);
      final cubit = UpcomingJobsCubit(coachRepository: mockCoachRepository);
      expect(cubit.state, isA<DataInitial<List<Event>>>());
      cubit.close();
    });
    ...
    blocTest<UpcomingJobsCubit, DataState<List<Event>>>(
      ...
      build: () {
        ...
        return UpcomingJobsCubit(coachRepository: mockCoachRepository);
      },
  ...
  group('EventsCubit', () {
    ...
    blocTest<EventsCubit, DataState<List<Event>>>(
      ...
      build: () {
        when(() => mockOrganiserRepository.getEvents())
            .thenAnswer((_) async => allFourEvents());
        return EventsCubit(organiserRepository: mockOrganiserRepository);
      },
    ...
    test('setAllowPastEvents(false) filters out past events', () async {
      when(() => mockOrganiserRepository.getEvents())
          .thenAnswer((_) async => allFourEvents());
      final cubit = EventsCubit(organiserRepository: mockOrganiserRepository);
  ...
  group('AuthCubit', () {
    ...
    test('login success emits AuthAuthenticated and sets currentUser', () async {
      when(() => mockAuthRepository.login(userLogin: testLogin)).thenAnswer(
      ...
      verify(() => mockAuthRepository.setToken('abc123')).called(1);
    ...
    test('login with network error emits AuthError', () async {
      when(() => mockAuthRepository.login(userLogin: testLogin))
    ...
    test('logout clears token/storage and emits AuthUnauthenticated', () async {
      ...
      verify(() => mockAuthRepository.setToken(null)).called(1);
      verify(() => mockStorage.clear()).called(1);

      cubit.close();
    });

    test('restoreSession validates token against backend', () async {
    ...
    test('restoreSession clears state when token is invalid', () async {
      when(() => mockStorage.getToken()).thenAnswer((_) async => 'expired_token');
      when(() => mockAuthRepository.setToken(any())).thenReturn(null);
      when(() => mockAuthRepository.getCurrentUser()).thenThrow(Exception('401'));
      when(() => mockStorage.clear()).thenAnswer((_) async {});
      ...
      // Should NOT be authenticated — token was invalid
      expect(cubit.state, isA<AuthInitial>());
      verify(() => mockStorage.clear()).called(1);
      verify(() => mockAuthRepository.setToken(null)).called(1);
  ...
  group('OrganiserCubit', () {
    ...
    test('initial state is the provided organiser', () {
      final cubit = OrganiserCubit(
        organiserRepository: mockOrganiserRepository,
    ...
    test('reload emits new organiser on success', () async {
      when(() => mockOrganiserRepository.getOrganiser())
